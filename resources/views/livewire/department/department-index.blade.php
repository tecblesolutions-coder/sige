<div class="space-y-4">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">Departamentos</h1>
            <p class="text-sm text-gray-500">Cat?logo de ?reas internas.</p>
        </div>
        <div class="flex flex-wrap items-center gap-3">
            ...
            <button wire:click="showDepartmentModal"
    ...
    @endif
    @if (session()->has('department-error'))
                ...
                <tbody class="divide-y divide-gray-100 text-sm dark:divide-gray-700">
                    @forelse ($departments as $key => $department)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/70">
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-200">{{ $departments->firstItem() + $key }}</td>
                            <td class="px-4 py-3 font-semibold text-gray-900 dark:text-gray-100">{{ $department->name }}</td>
                            <td class="px-4 py-3 text-gray-700 dark:text-gray-200 hidden sm:table-cell">{{ $department->code }}</td>
                        ...
                        <tr class="sm:hidden text-xs text-gray-600 dark:text-gray-300">
                            <td colspan="4" class="px-4 pb-4">
                                <div class="flex flex-wrap gap-3">
                                    <span class="flex items-center gap-1"><span class="font-semibold">C?digo:</span> {{ $department->code }}</span>
                                </div>
                            </td>
                        </tr>
                    @empty
    ...
    <div class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black/50 px-4" id="departmentModal">
        <div class="w-full max-w-lg bg-white rounded-xl shadow-xl dark:bg-gray-800 dark:border-gray-700">
            <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                    {{ $editMode ? 'Editar departamento' : 'Crear departamento' }}
                </h3>
            ...
            <div class="flex items-center justify-end gap-3 px-4 py-3 border-t border-gray-200 dark:border-gray-700">
                ...
                @if ($editMode)
                    <button class="px-4 py-2 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-500"
                        wire:click="updateDepartment">Actualizar</button>
                @else
                    <button class="px-4 py-2 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-500"
                        wire:click="storeDepartment">Guardar</button>
                @endif
            </div>
        </div>
    </div>
